...
Lidar_Data_Processing::Lidar_Data_Processing(){
  ...
  FirstSampleAngle = 0; //< Starting sampling angle
	LastSampleAngle = 0;  //< end sampling angle
  scan_frequence = 0;	      //Lidar speed in the protocol
  CheckSumResult = false;
  has_package_error = false;
  IntervalSampleAngle = 0.0;
	IntervalSampleAngle_LastPackage = 0.0;
  PackageSampleBytes = 2; //< The number of laser points contained in a package
  package_index = 0;
  recvNodeCount = 0;
  start_t = 0;
...
result_t Lidar_Data_Processing::waitSpeedRight(uint8_t cmd,uint64_t timeout)
{
    ...
    uint16_t data_check_sum = 0; //Accumulate data
    ...
    while ((waitTime = getms() - startTs) <= timeout)
    {
      ...
      for (size_t pos = 0; pos < recvSize; ++pos)
      {
        ...
        case 0:
          if (currentByte == 0x55)
          {
            //printf("Header data frame received\n");
          }
          else{
            continue;
          }
